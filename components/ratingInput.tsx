...
const RatingInput = () => {
  useEffect(() => {
    ...
    for (let i = 0; i < svgs.length; i++) {
      ...
      svgs[i].addEventListener("click", () => setRatingToBe(Number(valueStr)))
    }
  ...
  const [ratingToBe, setRatingToBe] = useState(0);
  ...
  return (
    <div className="flex items-center justify-center gap-1 my-4" id="ratingInputWrapper">
      <div className="flex relative">
        {getSvg(ratingPreview >= 1 || ratingToBe >= 1, false, 1)}
        {getSvg(ratingPreview >= 2 || ratingToBe >= 2, true, 2)}
      </div>
      <div className="flex relative">
        {getSvg(ratingPreview >= 3 || ratingToBe >= 3, false, 3)}
        {getSvg(ratingPreview >= 4 || ratingToBe >= 4, true, 4)}
      </div>
      <div className="flex relative">
        {getSvg(ratingPreview >= 5 || ratingToBe >= 5, false, 5)}
        {getSvg(ratingPreview >= 6 || ratingToBe >= 6, true, 6)}
      </div>
      <div className="flex relative">
        {getSvg(ratingPreview >= 7 || ratingToBe >= 7, false, 7)}
        {getSvg(ratingPreview >= 8 || ratingToBe >= 8, true, 8)}
      </div>
      <div className="flex relative">
        {getSvg(ratingPreview >= 9 || ratingToBe >= 9, false, 9)}
        {getSvg(ratingPreview >= 10 || ratingToBe >= 10, true, 10)}
      </div>
    </div>
  )
}
